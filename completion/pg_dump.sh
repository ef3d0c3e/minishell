complete -c pg_dump -s 'f' -l 'file' -d 'Output file or directory name'
complete -c pg_dump -s 'F' -l 'format' -d 'Output file format'
complete -c pg_dump -s 'j' -l 'jobs' -d 'Number of parallel jobs to dump'
complete -c pg_dump -s 'v' -l 'verbose' -d 'Verbose mode'
complete -c pg_dump -s 'V' -l 'version' -d 'Output version information'
complete -c pg_dump -s 'Z' -l 'compress' -d 'Compression level for compressed formats'
complete -c pg_dump -l 'lock-wait-timeout' -d 'Fail after waiting TIMEOUT for a table lock'
...
complete -c pg_dump -s 'E' -l 'encoding' -d 'Dump the data in encoding ENCODING'
complete -c pg_dump -s 'n' -l 'schema' -d 'Dump the named schemas only'
complete -c pg_dump -s 'N' -l 'exclude-schema' -d 'Do NOT dump the named schemas'
...
complete -c pg_dump -s 'S' -l 'superuser' -d 'Superuser user name to use in plain-text format'
complete -c pg_dump -s 't' -l 'table' -d 'Dump the named tables only'
complete -c pg_dump -s 'T' -l 'exclude-table' -d 'Do NOT dump the named tables'
...
complete -c pg_dump -l 'exclude-table-data' -d 'Do NOT dump data for the named tables'
...
complete -c pg_dump -l 'section' -d 'Dump named section (pre-data, data, or post-data)'
complete -c pg_dump -l 'serializable-deferrable' -d 'Wait until the dump can run without anomalies'
complete -c pg_dump -l 'snapshot' -d 'Use given snapshot for the dump'
...
complete -c pg_dump -s 'd' -l 'dbname' -d 'Database to dump'
complete -c pg_dump -s 'h' -l 'host' -d 'Database server host or socket directory'
complete -c pg_dump -s 'p' -l 'port' -d 'Database server port number'
complete -c pg_dump -s 'U' -l 'username' -d 'Connect as specified database user'
...
complete -c pg_dump -l 'role' -d 'Do SET ROLE before dump'
