...
complete -c tree -s 'L' -d 'Descend only level directories deep'
complete -c tree -s 'R' -d 'Rerun tree when max dir level reached'
complete -c tree -s 'P' -d 'List only those files that match the pattern given'
complete -c tree -s 'I' -d 'Do not list files that match the given pattern'
...
complete -c tree -l 'charset' -d 'Use charset X for terminal/HTML and indentation line output'
complete -c tree -l 'filelimit' -d 'Do not descend dirs with more than # files in them'
complete -c tree -s 'o' -d 'Output to file instead of stdout'
...
complete -c tree -l 'timefmt' -d 'Print and format time according to the format <f>'
...
complete -c tree -l 'sort' -d 'Select sort'
...
complete -c tree -s 'H' -d 'Prints out HTML format with baseHREF as top directory'
complete -c tree -s 'T' -d 'Replace the default HTML title and H1 header with string'
complete -c tree -l 'nolinks' -d 'Turn off hyperlinks in HTML output'
complete -c tree -l 'fromfile' -d 'Reads paths from files (.=stdin)'
