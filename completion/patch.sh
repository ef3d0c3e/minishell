...
complete -c patch -s 'B' -l 'prefix' -d 'Prepend PREFIX to backup filenames'
complete -c patch -s 'c' -l 'context' -d 'Interpret patch as context diff'
complete -c patch -s 'd' -l 'directory' -d 'Change directory to DIR first'
complete -c patch -s 'D' -l 'ifdef' -d 'Make merged if-then-else output using NAME'
...
complete -c patch -s 'F' -l 'fuzz' -d 'Number of LINES for inexact '"'"'fuzzy'"'"' matching'
complete -c patch -s 'g' -l 'get' -d 'Get files from RCS etc. if positive; ask if negative'
complete -c patch -l 'help' -d 'Display help'
complete -c patch -s 'i' -l 'input' -d 'Read patch from FILE instead of stdin'
...
complete -c patch -s 'o' -l 'output' -d 'Output patched files to FILE'
complete -c patch -s 'p' -l 'strip' -d 'Strip NUM path components from filenames'
complete -c patch -l 'posix' -d 'Conform to the POSIX standard'
complete -c patch -l 'quoting-style' -d 'Output file names using quoting style'
complete -c patch -s 'r' -l 'reject-file' -d 'Output rejects to FILE'
...
complete -c patch -s 'V' -l 'version-control' -d 'Use method to determine backup file names'
complete -c patch -l 'verbose' -d 'Output extra information about work being done'
complete -c patch -s 'x' -l 'debug' -d 'Set internal debugging flags'
complete -c patch -s 'Y' -l 'basename-prefix' -d 'Prepend PREFIX to backup file basenames'
complete -c patch -s 'z' -l 'suffix' -d 'Append SUFFIX to backup file name'
