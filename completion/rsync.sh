...
complete -c rsync -l 'stderr' -d 'change stderr output mode, default: errors'
...
complete -c rsync -l 'cc' -l 'checksum-choice' -d 'Choose the checksum algorithm'
complete -c rsync -s 'x' -l 'one-file-system' -d 'Don’t cross filesystem boundaries'
complete -c rsync -s 'B' -l 'block-size' -d 'Force a fixed checksum block-size'
complete -c rsync -s 'e' -l 'rsh' -d 'Specify the remote shell to use'
complete -c rsync -l 'rsync-path' -d 'Specify the rsync to run on remote machine'
...
complete -c rsync -l 'max-delete' -d 'Don’t delete more than NUM files'
complete -c rsync -l 'max-size' -d 'Don’t transfer any file larger than SIZE'
complete -c rsync -l 'min-size' -d 'Don’t transfer any file smaller than SIZE'
complete -c rsync -l 'max-alloc' -d 'Change process memory allocation limit'
complete -c rsync -l 'partial' -d 'Keep partially transferred files'
complete -c rsync -l 'partial-dir' -d 'Put a partially transferred file into DIR'
...
complete -c rsync -l 'usermap' -d 'Custom username mapping'
complete -c rsync -l 'groupmap' -d 'Custom username mapping'
complete -c rsync -l 'chown' -d 'Combined username/groupname mapping'
...
complete -c rsync -l 'modify-window' -d 'Compare NUM mod-times with reduced accuracy'
complete -c rsync -s 'T' -l 'temp-dir' -d 'Create temporary files in directory DIR'
complete -c rsync -s 'y' -l 'fuzzy' -d 'Find similar file for basis if no dest file'
complete -c rsync -l 'compare-dest' -d 'Also compare received files relative to DIR'
complete -c rsync -l 'copy-dest' -d 'Like compare-dest but also copies unchanged files'
complete -c rsync -l 'link-dest' -d 'Hardlink to files in DIR when unchanged'
complete -c rsync -s 'z' -l 'compress' -d 'Compress file data during the transfer'
complete -c rsync -l 'zc' -l 'compress-choice' -d 'Choose the compression algorithm'
complete -c rsync -l 'compress-level' -d 'Explicitly set compression level (aka --zl)'
complete -c rsync -l 'skip-compress' -d 'Skip compressing files with suffix in LIST'
...
complete -c rsync -l 'include-from' -d 'Read include patterns from FILE'
complete -c rsync -l 'files-from' -d 'Read list of source-file names from FILE'
...
complete -c rsync -l 'copy-as' -d 'Specify user & optional group for the copy'
...
complete -c rsync -l 'outbuf' -d 'Set out buffering to None, Line, or Block'
...
complete -c rsync -l 'out-format' -d 'Output updates using the specified FORMAT'
complete -c rsync -l 'log-file' -d 'log what we'"'"'re doing to the specified FILE'
complete -c rsync -l 'log-file-format' -d 'log updates using the specified FMT'
complete -c rsync -l 'password-file' -d 'Read password from FILE'
...
complete -c rsync -l 'bwlimit' -d 'Limit I/O bandwidth; optional unit (KB/s default)'
complete -c rsync -l 'stop-after' -d 'Stop rsync after MINS minutes have elapsed'
complete -c rsync -l 'stop-at' -d 'Stop rsync at the specified point in time'
complete -c rsync -l 'write-batch' -d 'Write a batched update to FILE'
complete -c rsync -l 'only-write-batch' -d 'Like --write-batch but w/o updating dest'
complete -c rsync -l 'read-batch' -d 'Read a batched update from FILE'
complete -c rsync -l 'protocol' -d 'Force an older protocol version to be used'
complete -c rsync -l 'iconv' -d 'Request charset conversion of filenames'
complete -c rsync -l 'checksum-seed' -d 'Set block/file checksum seed (advanced)'
...
complete -c rsync -l 'config' -d 'Specify alternate rsyncd.conf file'
complete -c rsync -l 'dparam' -d 'Override global daemon config parameter'
complete -c rsync -l 'no-detach' -d 'Do not detach from the parent'
