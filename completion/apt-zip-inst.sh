...
complete -c apt-zip-inst -s 'V' -l 'version' -d 'Display version and exit'
complete -c apt-zip-inst -s 'm' -l 'medium' -d 'Removable medium'
complete -c apt-zip-inst -s 'a' -l 'aptgetaction' -d 'Select an action'
complete -c apt-zip-inst -s 'p' -l 'packages' -d 'List of packages to install'
complete -c apt-zip-inst -s 'f' -l 'fix-broken' -d 'Fix broken option'
