complete -c psql -s 'd' -l 'dbname' -d 'Database name to connect to'
complete -c psql -s 'c' -l 'command' -d 'Run only single command (SQL or internal) and exit'
complete -c psql -s 'f' -l 'file' -d 'Execute commands from file, then exit'
complete -c psql -s 'l' -l 'list' -d 'List available databases, then exit'
complete -c psql -s 'v' -l 'set' -l 'variable' -d 'Set psql variable NAME to VALUE'
...
complete -c psql -s '?' -l 'help' -d 'Show this help, then exit'
...
complete -c psql -s 'h' -l 'host' -d 'Database server host or socket directory'
complete -c psql -s 'p' -l 'port' -d 'Database server port'
complete -c psql -s 'U' -l 'username' -d 'Database user name'
