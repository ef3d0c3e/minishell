...
complete -c zstd -l 'train' -d 'Create a dictionary from specified file(s)'
...
complete -c zstd -l 'threads=0' -d 'Compress using as many threads as there are CPU cores on the system'
...
complete -c zstd -s 'D' -d 'Use specified file as dictionary'
complete -c zstd -l 'no-dictID' -d 'Do not write dictID into header'
complete -c zstd -s 'o' -d 'Specify file to save'
