...
complete -c nodeenv -s 'n' -l 'node' -d 'The Node.js version to use'
complete -c nodeenv -l 'mirror' -d 'The mirror of nodejs.org to download from'
complete -c nodeenv -s 'j' -l 'jobs' -d 'The number of compile jobs to run in parallel'
complete -c nodeenv -l 'load-average' -d 'The maximum load average during parallel compilation.'
...
complete -c nodeenv -l 'make' -s 'm' -d 'Path to make command'
...
complete -c nodeenv -s 'C' -l 'config-file' -d 'Config file to use instead of ~/.nodeenvrc'
complete -c nodeenv -s 'r' -l 'requirements' -d 'Install all Node.js packages listed in this file'
complete -c nodeenv -l 'prompt' -d 'Set a custom prompt prefix for this environment'
...
complete -c nodeenv -l 'npm' -d 'The npm version to use'
