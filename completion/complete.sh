complete -c complete -s 'c' -l 'command' -d 'Command to add completion to'
...
complete -c complete -s 'a' -l 'arguments' -d 'Space-separated list of possible arguments'
...
complete -c complete -s 'n' -l 'condition' -d 'Completion only used if command has zero exit status'
