complete -c atool -d 'Manage file archives of various types'
...
complete -c atool -s 'X' -l 'extract-to' -d 'extract archive to specified directory'
...
complete -c atool -s 'F' -l 'format' -d 'override archive format (see below)'
complete -c atool -s 'O' -l 'format-option' -d 'give specific options to the archiver'
...
complete -c atool -s 'V' -l 'verbosity' -d 'specify verbosity (0, 1 or 2)'
...
complete -c atool -s 'o' -l 'option' -d 'override a configuration option (KEY=VALUE)'
complete -c atool -l 'config' -d 'load configuration defaults from file'
