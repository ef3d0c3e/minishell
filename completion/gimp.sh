...
complete -c gimp -l 'display' -d 'open with X display'
complete -c gimp -l 'session' -d 'open with alternative sessionrc'
complete -c gimp -s 'g' -l 'gimprc' -d 'open with alternative gimprc'
complete -c gimp -l 'system-gimprc' -d 'open with alternative system gimprc'
...
complete -c gimp -l 'stack-trace-mode' -d 'whether generate stack-trace in case of fatal signals'
complete -c gimp -l 'pdb-compat-mode' -d 'whether PDB provides aliases for deprecated functions'
complete -c gimp -l 'batch-interpreter' -d 'run procedure to use to process batch events'
complete -c gimp -s 'b' -l 'batch' -d 'run command non-interactively'
