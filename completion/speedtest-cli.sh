...
complete -c speedtest-cli -l 'csv-delimiter' -d 'Single character delimiter to use in CSV output'
...
complete -c speedtest-cli -l 'server' -d 'Specify a server ID to test against'
complete -c speedtest-cli -l 'exclude' -d 'Exclude a server from selection'
complete -c speedtest-cli -l 'mini' -d 'URL of the Speedtest Mini server'
complete -c speedtest-cli -l 'source' -d 'Source IP address to bind to'
complete -c speedtest-cli -l 'timeout' -d 'HTTP timeout in seconds'
