...
complete -c zip -d 'Convert CR LF to LF'
...
complete -c zip -s 'x' -d 'Exclude the following names'
complete -c zip -s 'i' -d 'Include only the following names'
complete -c zip -s 'F' -d 'Fix zipfile'
complete -c zip -d 'Fix zipfile (try harder)'
...
complete -c zip -s 'n' -d 'Don'"'"'t compress files with these suffixes'
