...
complete -c fsharpi -l 'reference:' -d 'Reference an assembly'
complete -c fsharpi -s 'g' -l 'debug' -l 'debug+' -d 'Emit debug information'
complete -c fsharpi -l 'debug-' -d 'Disable --debug'
complete -c fsharpi -s 'O' -l 'optimize' -l 'optimize+' -d 'Enable optimizations'
complete -c fsharpi -l 'optimize-' -d 'Disable --optimize'
...
complete -c fsharpi -l 'define:' -d 'Define conditional compilation symbols'
...
complete -c fsharpi -l 'lib:' -d 'Specify a dir for the include path for source files and assemblies'
