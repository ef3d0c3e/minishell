...
complete -c scons -l 'debug' -d 'Debug the build process'
complete -c scons -l 'diskcheck' -d 'Check if files and directories are where they should be'
...
complete -c scons -l 'duplicate' -d 'How to duplicate files'
...
complete -c scons -l 'warn' -d 'Enable or disable warnings'
