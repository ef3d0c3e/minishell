complete -c dumpcap -s 'C' -d 'Limit the amount of memory in bytes for storing captured packets in memory'
...
complete -c dumpcap -s 'N' -d 'Limit the number of packets used for storing captured packets in memory'
