...
complete -c web-ext -d 'Create the extension package from source'
complete -c web-ext -d 'Run the config discovery and dump the resulting config data as JSON'
complete -c web-ext -d 'Sign the extension so it can be installed in Firefox'
complete -c web-ext -d 'Run the extension'
complete -c web-ext -d 'Validate the extension source'
complete -c web-ext -d 'Show help in the browser'
