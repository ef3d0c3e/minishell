...
complete -c mysql -l 'character-sets-dir' -d 'Set directory where character sets are installed'
...
complete -c mysql -l 'connect-timeout' -d 'Set the number of seconds before connection timeout'
...
complete -c mysql -l 'default-auth' -d 'Default authentication client-side plugin'
complete -c mysql -l 'default-character-set' -d 'Use charset_name as the default character set'
complete -c mysql -l 'defaults-extra-file' -d 'Read default options from file after global defaults'
complete -c mysql -l 'defaults-file' -d 'Read default options from file overriding global defaults'
complete -c mysql -l 'defaults-group-suffix' -d 'Read groups that have the given suffix'
complete -c mysql -l 'delimiter' -d 'Set the statement delimiter'
complete -c mysql -l 'disable-named-commands' -d 'Disable named commands'
complete -c mysql -s 'e' -l 'execute' -d 'Execute the statement'
complete -c mysql -s 'f' -l 'force' -d 'Continue even if an SQL error occurs'
complete -c mysql -s 'h' -l 'host' -d 'Connect to the server on the given host'
...
complete -c mysql -l 'init-command' -d 'SQL Command to execute when connecting to the server'
complete -c mysql -l 'line-numbers' -d 'Write line numbers for errors'
complete -c mysql -l 'local-infile' -d 'Enable or disable LOCAL capability for LOAD DATA INFILE'
complete -c mysql -l 'max-allowed-packet' -d 'Set the maximum packet length'
complete -c mysql -l 'max-join-size' -d 'Set the automatic limit for rows in a join'
...
complete -c mysql -l 'net-buffer-length' -d 'Set the buffer size for TCP/IP and socket communication'
...
complete -c mysql -l 'plugin-dir' -d 'Directory for client-side plugins'
complete -c mysql -s 'P' -l 'port' -d 'The port to use for the connection'
...
complete -c mysql -l 'prompt' -d 'Set the prompt to the specified format'
complete -c mysql -l 'protocol' -d 'The connection protocol to use'
...
complete -c mysql -l 'select-limit' -d 'Set automatic limit for SELECT when using --safe-updates'
complete -c mysql -l 'server-arg' -d 'Send a parameter to the embedded server'
...
complete -c mysql -s 'S' -l 'socket' -d 'For connections to localhost the socket file or named pipe'
complete -c mysql -l 'ssl' -d 'Enable SSL for connection'
complete -c mysql -l 'skip-ssl' -d 'Disable SSL for connection'
complete -c mysql -l 'ssl-ca' -d 'CA file in PEM format'
complete -c mysql -l 'ssl-capath' -d 'CA directory'
complete -c mysql -l 'ssl-cert' -d 'X509 cert in PEM format'
complete -c mysql -l 'ssl-cipher' -d 'SSL cipher to use'
complete -c mysql -l 'ssl-key' -d 'X509 key in PEM format'
complete -c mysql -l 'ssl-crl' -d 'Certificate revocation list'
complete -c mysql -l 'ssl-crlpath' -d 'Certificate revocation list path'
...
complete -c mysql -s 'u' -l 'user' -d 'User name to use when connecting'
