...
complete -c read -s 'p' -l 'prompt' -d 'Set prompt command'
complete -c read -s 'P' -l 'prompt-str' -d 'Set prompt using provided string'
...
complete -c read -s 'c' -l 'command' -d 'Initial contents of read buffer when reading interactively'
...
complete -c read -s 'R' -l 'right-prompt' -d 'Set right-hand prompt command'
...
complete -c read -s 'd' -l 'delimiter' -d 'Set string to use as delimiter'
