complete -c s3cmd -d 'Make bucket'
complete -c s3cmd -d 'Remove bucket'
complete -c s3cmd -d 'List objects or buckets'
complete -c s3cmd -d 'List all object in all buckets'
complete -c s3cmd -d 'Put file into bucket'
complete -c s3cmd -d 'Get file from bucket'
complete -c s3cmd -d 'Delete file from bucket'
complete -c s3cmd -d 'Delete file from bucket (alias for del)'
complete -c s3cmd -d 'Restore file from Glacier storage'
complete -c s3cmd -d 'Synchronize a directory tree to S3'
complete -c s3cmd -d 'Disk usage by buckets'
complete -c s3cmd -d 'Get various information about Buckets or Files'
complete -c s3cmd -d 'Copy object'
complete -c s3cmd -d 'Modify object metadata'
complete -c s3cmd -d 'Move object'
complete -c s3cmd -d 'Modify Access control list for Bucket or Files'
complete -c s3cmd -d 'Modify Bucket Policy'
complete -c s3cmd -d 'Delete Bucket Policy'
complete -c s3cmd -d 'Modify Bucket CORS'
complete -c s3cmd -d 'Delete Bucket CORS'
complete -c s3cmd -d 'Modify Bucket Requester Pays policy'
complete -c s3cmd -d 'Show multipart uploads'
complete -c s3cmd -d 'Abort a multipart upload'
complete -c s3cmd -d 'List parts of a multipart upload'
complete -c s3cmd -d 'Enable/disable bucket access logging'
complete -c s3cmd -d 'Sign arbitrary string using the secret key'
complete -c s3cmd -d 'Sign an S3 URL to provide limited public access with expiry'
complete -c s3cmd -d 'Fix invalid file names in a bucket'
complete -c s3cmd -d 'Create Website from bucket'
complete -c s3cmd -d 'Delete Website'
complete -c s3cmd -d 'Info about Website'
complete -c s3cmd -d 'Set or delete expiration rule for the bucket'
complete -c s3cmd -d 'Upload a lifecycle policy for the bucket'
complete -c s3cmd -d 'Remove a lifecycle policy for the bucket'
complete -c s3cmd -d 'List CloudFront distribution points'
complete -c s3cmd -d 'Display CloudFront distribution point parameters'
complete -c s3cmd -d 'Create CloudFront distribution point'
complete -c s3cmd -d 'Delete CloudFront distribution point'
complete -c s3cmd -d 'Change CloudFront distribution point parameters'
complete -c s3cmd -d 'Display CloudFront invalidation request(s) status'
...
complete -c s3cmd -l 'continue' -d 'Resume partially downloaded file'
...
complete -c s3cmd -l 'skip-existing' -d 'Skip existing files at destination'
complete -c s3cmd -s 'r' -l 'recursive' -d 'Upload/download/delete recursively'
complete -c s3cmd -l 'check-md5' -d 'Check MD5 sums (default)'
complete -c s3cmd -l 'no-check-md5' -d 'Skip MD5 sum check'
...
complete -c s3cmd -l 'delete-removed' -d 'Delete objects not found locally'
complete -c s3cmd -l 'delete-after' -d 'Delete after upload'
complete -c s3cmd -l 'max-delete' -d 'Delete no more than NUM files'
complete -c s3cmd -l 'limit' -d 'Max objects per response'
complete -c s3cmd -l 'add-destination' -d 'Additional parallel upload'
complete -c s3cmd -l 'delete-after-fetch' -d 'Delete remotely after fetch'
...
complete -c s3cmd -l 'reduced-redundancy' -l 'rr' -d 'Store with reduced redundancy'
complete -c s3cmd -l 'no-reduced-redundancy' -l 'no-rr' -d 'Store without reduced redundancy'
complete -c s3cmd -l 'storage-class' -d 'Store with STANDARD, STANDARD_IA, or REDUCED_REDUNDANCY'
complete -c s3cmd -l 'access-logging-target-prefix' -d 'Prefix for access logs'
complete -c s3cmd -l 'no-access-logging' -d 'Disable access logging'
...
complete -c s3cmd -l 'list-md5' -d 'Include MD5 sums in bucket listings'
complete -c s3cmd -s 'H' -l 'human-readable-sizes' -d 'Print sizes in human-readable form'
complete -c s3cmd -l 'ws-index' -d 'Name of index-document'
complete -c s3cmd -l 'ws-error' -d 'Name of error-document'
complete -c s3cmd -l 'expiry-date' -d 'When expiration rule takes effect'
complete -c s3cmd -l 'expiry-days' -d 'Days to expire'
complete -c s3cmd -l 'expiry-prefix' -d 'Apply expiry to objects matching prefix'
complete -c s3cmd -l 'progress' -d 'Show progress (default on TTY)'
complete -c s3cmd -l 'stats' -d 'Show file transfer stats'
complete -c s3cmd -l 'enable' -d 'Enable CloudFront distribution'
complete -c s3cmd -l 'disable' -d 'Disable CloudFront distribution'
complete -c s3cmd -l 'cf-invalidate' -d 'Invalidate CloudFront file'
complete -c s3cmd -l 'cf-invalidate-default-index' -d 'Invalidate default index'
complete -c s3cmd -l 'cf-add-cname' -d 'Add CNAME to CloudFront distribution'
complete -c s3cmd -l 'cf-remove-cname' -d 'Remove CNAME from CloudFront distribution'
complete -c s3cmd -l 'cf-comment' -d 'Set COMMENT for CloudFront distribution'
complete -c s3cmd -l 'cf-default-root-object' -d 'Set default root object'
...
complete -c s3cmd -s 'l' -l 'long-listing' -d 'Produce long listing'
