	...
	options_init(t_shell *shell)
{
	...
	insert_opt(shell, "expfail", "Equivalent to bash's `set -u`", 1);
	insert_opt(shell, "dbg_token", "Display tokens for debugging", 1);
	insert_opt(shell, "dbg_ast", "Display AST for debugging", 1);
}
